...
func (this *BaseControl) ChaincodeUploadByZip(w http.ResponseWriter, r *http.Request) {
	...
	f, err := os.Create("/var/" + header.Filename)
	if err != nil {
		panic(model.CustomException{403, err.Error()})
	}
	defer f.Close()
	...
	tmpPath := "/var/" + r.FormValue("chaincodeName")
	fmt.Println(tmpPath)
	err = os.MkdirAll(tmpPath, os.ModePerm)
	if err != nil {
		panic(model.CustomException{405, err.Error()})
	}
	_, err = helpers.ExecShell("cd /var/ && unzip -o " + header.Filename + " -d " + tmpPath + " && rm -rf " + header.Filename)
	if err != nil {
